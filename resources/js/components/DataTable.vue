...
const table = useVueTable<any>({
  ...
  get pageCount() {
    return (props.manualPagination ?? false) ? pageCountValue.value : undefined
  },
  onPaginationChange: (updater) => {
    const current = { pageIndex: pageIndex.value, pageSize: pageSize.value }
    const nextState = typeof updater === 'function' ? updater(current) : (updater as any)
    if (typeof nextState.pageIndex === 'number' && nextState.pageIndex !== pageIndex.value) {
      pageIndex.value = nextState.pageIndex
      emit('update:pageIndex', pageIndex.value)
    }
    if (typeof nextState.pageSize === 'number' && nextState.pageSize !== pageSize.value) {
...
const pageInfo = computed(() => {
  ...
  const pageCount = props.manualPagination
    ? (props.pageCount === -1 ? current : props.pageCount)
    : table.getPageCount()
  return { current, pageCount }
})

const totalRows = computed(() => {
  if (props.manualPagination && props.totalCount != null) return props.totalCount
  ...
  <div class="space-y-3">
    <div v-if="enableGlobalFilter" class="flex items-center gap-2">
      <UiInput
        ...
        class="max-w-xs"
      />
      <UiButton variant="outline" size="sm" @click="() => table.resetColumnFilters()">Reset filters</UiButton>
      <div class="ml-auto flex items-center gap-2">
        <label for="per-page" class="text-sm text-muted-foreground">Rows per page</label>
        <select
          id="per-page"
        ...
        >
          <option v-for="size in pageSizesTyped" :key="Number(size)" :value="Number(size)">{{ size }}</option>
        </select>
      </div>
    </div>
    ...
    <div :class="tableClass">
      <table class="w-full text-sm">
        <thead class="bg-muted/50">
          <tr v-for="headerGroup in table.getHeaderGroups()" :key="String(headerGroup.id)">
            <th
              ...
              <span class="inline-flex items-center gap-1">
                <span>
                  <template v-if="!header.isPlaceholder">
                    <FlexRender :render="header.column.columnDef.header" v-bind="header.getContext()" />
                  </template>
                </span>
                <span v-if="header.column.getCanSort()" class="text-xs text-muted-foreground">{{ sortIndicator(header.column) }}</span>
              </span>
            </th>
          </tr>
          <!-- Column filters row -->
        ...
        <tbody>
          <template v-if="table.getRowModel().rows.length">
            ...
            >
              <td v-for="cell in row.getVisibleCells()" :key="String(cell.id)" class="px-3 py-2">
                <FlexRender :render="cell.column.columnDef.cell" v-bind="cell.getContext()" />
              </td>
            </tr>
          </template>
          <tr v-else>
            <td :colspan="table.getAllColumns().length" class="px-3 py-6 text-center text-muted-foreground">
              No results.
            </td>
          </tr>
        </tbody>
    ...
    <div class="flex items-center justify-between gap-3 text-sm">
      <div class="text-muted-foreground">
        Showing
        <span class="font-medium">{{ table.getRowModel().rows.length ? (pageIndex * pageSize) + 1 : 0 }}</span>
        to
        <span class="font-medium">{{ Math.min((pageIndex + 1) * pageSize, totalRows) }}</span>
        of
      ...
      <div class="flex items-center gap-2">
        <UiButton variant="outline" size="sm" :disabled="!table.getCanPreviousPage()" @click="table.setPageIndex(0)">
          « First
        </UiButton>
        ...
        <UiButton variant="outline" size="sm" :disabled="!table.getCanNextPage()" @click="table.setPageIndex(table.getPageCount() - 1)">
          Last »
        </UiButton>
      </div>
    </div>
