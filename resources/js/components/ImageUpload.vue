...
<template>
  <div class="space-y-2">
    ...
    <div class="relative group" v-if="hasPreview">
      <img :src="previewUrl as string" :class="['rounded-md border object-contain bg-white', previewClass ?? 'w-full h-32']" />
      <button
        type="button"
        class="absolute inset-0 hidden place-items-center rounded-md bg-black/50 text-white group-hover:grid cursor-pointer"
        @click="onRemove"
      >
        Remove
      </button>
